 ...
 *   id = "link_uri_translation"
 * )
 */
class LinkUriTranslation extends ProcessPluginBase implements ContainerFactoryPluginInterface {
    ...
    if (parse_url($path, PHP_URL_SCHEME) === NULL) {
      ...
      if ($url->isRouted()) {
        ...
        if (!$this->configuration['validate_route']) {
          ...
          $path = 'internal:/node/' . $row->getSourceProperty('tnid');
          $url = Url::fromUri($path);
          ...
          if ($url->isRouted()) {
            ...
            throw new MigrateException(str_replace("internal:/node/", "", $path));
          }
        }
      }
    }
