...
class VwBuildForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $lists = $osha_config->get('lists');
    if (!isset($lists)) {
      $tables = $this->database->schema()->findTables('osha_%');
      ...
      foreach ($osha_config->get('lists')['list'] as $list) {
        ...
        if (boolval($form_state->getValue('rebuild'))) {
          $this->database->schema()->dropTable('osha_workflow_' . $name);
        }

        // Create the table.
        if (empty($this->database->schema()->tableExists('osha_workflow_' . $name))) {
          $this->database->schema()->createTable('osha_workflow_' . $name, [
            'fields' => [
              'id' => [
                'type' => 'serial',
                'not null' => TRUE,
